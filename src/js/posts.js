posts = [
  {
    title: "Check out this awesome HTML5 trick!",
    body: "Hey folks, just stumbled upon this cool HTML5 trick and wanted to share it with you all. Did you know that you can use the 'video' tag to embed videos directly into your web page? It's super easy and adds a nice touch to your site. Give it a try!",
    author: "joolzie123",
    profileLink: "../views/profiles/view-profile-view.html",
    date: "06/15/2023",
    totalVote: "15",
    totalCommentCount: "3",
    comments: [
      {
        user: "cirup29",
        date: "06/16/2023",
        text: "Whoa, that's neat! I'll definitely try it out on my next project. Thanks for sharing, joolzie123!",
      },
      {
        user: "coco_san00",
        date: "06/17/2023",
        text: "That's a great tip! Videos can really enhance the user experience. I've used it before, and it worked like a charm.",
      },
      {
        user: "coc_man_2099",
        date: "06/18/2023",
        text: "Nice find, can you also share any resources or tutorials to help us implement it effectively?",
      },
    ],
  },
  {
    title: "Favorite Binge-Worthy TV Shows",
    body: "Calling all TV addicts! Let's talk about our favorite binge-worthy TV shows. Which series have kept you glued to the screen, eagerly clicking that \"Next Episode\" button? Whether it's gripping dramas, hilarious comedies, or thrilling sci-fi, share your top picks and let's indulge in some television talk!",
    author: "coc_man_2099",
    profileLink: "../views/profiles/coc_man_2099.html",
    date: "04/15/2023",
    totalVote: "10",
    totalCommentCount: "3",
    comments: [
      {
        user: "cirup29",
        date: "04/15/2023",
        text: "Brooklyn Nine-Nine is my go-to comedy series. The hilarious cast and witty writing never fail to put a smile on my face.",
      },
      {
        user: "coco_san00",
        date: "04/16/2023",
        text: "If you haven't watched Breaking Bad yet, you're missing out! The intense storyline and brilliant performances make it an absolute must-watch.",
      },
      {
        user: "gundamn",
        date: "04/21/2023",
        text: "Stranger Things is a fantastic sci-fi show that captures the '80s nostalgia perfectly. The suspenseful plot and lovable characters make it a binge-worthy delight.",
      },
    ],
  },
  {
    title: "Fitness Tips for Beginners",
    body: "Attention fitness enthusiasts! Let's help out the beginners on their fitness journey. If you have any valuable tips, advice, or resources for someone who's just starting to embrace a healthier lifestyle, share them here. From workout routines to healthy eating habits or even mental wellbeing, let's empower each other to lead healthier lives!",
    author: "coco_san00",
    profileLink: "../views/profiles/coco_san00.html",
    date: "05/01/2020",
    totalVote: "2",
    totalCommentCount: "3",
    comments: [
      {
        user: "joolzie123",
        date: "05/01/2020",
        text: "Consistency is key! Start with small achievable goals and gradually increase the intensity. Remember, progress is progress, no matter how small.",
        replies: [
          {
            user: "coc_man_2099",
            date: "05/02/2020",
            text: "Don't forget to prioritize mental wellbeing alongside physical fitness. Incorporating mindfulness practices like yoga or meditation can bring balance and reduce stress.",
          },
        ],
      },
      {
        user: "cirup29",
        date: "05/01/2020",
        text: "Don't underestimate the power of nutrition. Focus on incorporating whole, unprocessed foods into your diet and hydrate well. It's a game-changer for your overall health and fitness.",
      },
    ],
  },
  {
    title: 'Share Your Funniest "Auto-correct Fails"',
    body: "Auto-correct can be both a lifesaver and a source of hilarious confusion. Share your funniest auto-correct fails or embarrassing texting mishaps that left you in stitches. Let's commiserate and laugh together over the perils of technology!",
    author: "cirup29",
    profileLink: "../views/profiles/cirup29.html",
    date: "06/20/2023",
    totalVote: "6",
    totalCommentCount: "3",
    comments: [
      {
        user: "gundamn",
        date: "06/20/2023",
        text: 'I once texted my friend, "I\'m heading to the bar for drinks." Auto-correct changed it to "I\'m heading to the bat for drinks." Needless to say, it raised some eyebrows!',
      },
      {
        user: "coco_san00",
        date: "06/25/2023",
        text: 'I once sent my mom a text saying, "Dinner will be owl-ready." Auto-correct turned "owl-ready" into "bowling." She thought we were having a bowling party at home!',
      },
      {
        user: "joolzie123",
        date: "06/27/2023",
        text: 'My friend texted me, "I\'ll bring the wime." Auto-correct transformed "wime" into "mime." We ended up having a good laugh about picturing a silent mime at the party!',
      },
    ],
  },
  {
    title: "Share Your Funniest Programming Jokes",
    body: "Calling all coders with a sense of humor! Let's lighten up the coding world with some good-natured jokes. Share your funniest programming jokes or hilarious coding anecdotes. It's time to bring a smile to our fellow developers' faces!",
    author: "joolzie123",
    profileLink: "../views/profiles/view-profile-view.html",
    date: "02/01/2021",
    totalVote: "7",
    totalCommentCount: "3",
    comments: [
      {
        user: "coco_san00",
        date: "02/01/2021",
        text: 'I once texted my friend, "I\'m heading to the bar for drinks." Auto-correct changed it to "I\'m heading to the bat for drinks." Needless to say, it raised some eyebrows!',
      },
      {
        user: "cirup29",
        date: "02/01/2021",
        text: 'I once sent my mom a text saying, "Dinner will be owl-ready." Auto-correct turned "owl-ready" into "bowling." She thought we were having a bowling party at home!',
      },
      {
        user: "gundamn",
        date: "02/01/2021",
        text: 'My friend texted me, "I\'ll bring the wime." Auto-correct transformed "wime" into "mime." We ended up having a good laugh about picturing a silent mime at the party!',
      },
    ],
  },
  {
    title: "Laufey in Manila",
    body: "Just got back from the Laufey concert in Manila and WOW! It was a mind blowing experience. I’ve been a fan for a while now and it was surreal to see her perform live. Laufey's voice is even more enchanting in person. The atmosphere was electric and everyone was singing along- Not to mention the set list was *chef’s kiss* Who else attended? Share your experiences!",
    author: "joolzie123",
    profileLink: "../views/profiles/view-profile-view.html",
    date: "05/30/2023",
    totalVote: "23",
    totalCommentCount: "3",
    comments: [
      {
        user: "coco_san00",
        date: "05/30/2023",
        text: "Oh, I wish I could've been there! How was the concert? I've been a fan of Laufey for a while now, and I can only imagine how amazing it must have been to see them perform live.",
      },
      {
        user: "cirup29",
        date: "05/30/2023",
        text: "I'm so jealous! I've been obsessed with Laufey's debut EP, and I can only imagine how mesmerizing the live performance was",
      },
    ],
  },
];
